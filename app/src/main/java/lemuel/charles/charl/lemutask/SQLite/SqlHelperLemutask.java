package lemuel.charles.charl.lemutask.SQLite;
...
public class SqlHelperLemutask extends SQLiteOpenHelper {
    ...
    public static final String COL19 = "ORIGINALTIME";//original time so that time can be sorted out,
    // when selecting classes to list view
    ...
    public SqlHelperLemutask(Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME +
                ...
                " DESCRIPTION TEXT, STIME TEXT, ETIME TEXT, ISTIMED INTEGER, " +
                "ISDONE INTEGER, DAY INTEGER, MONTH INTEGER, YEAR INTEGER, TDATE DATE)");

        db.execSQL("CREATE TABLE IF NOT EXISTS " + CLASSES_TABLE_NAME +
                " ( _id INTEGER PRIMARY KEY AUTOINCREMENT, TITLE TEXT, CODE TEXT," +
                " CREDIT INTEGER, VENUE TEXT, LECTURERS TEXT, SRTCLASSTIME TEXT, " +
                "STPCLASSTIME TEXT, ORIGINALTIME INTEGER, DAY_C TEXT)");

        db.execSQL("CREATE TABLE IF NOT EXISTS " + ASSIGNMENT_TABLE_NAME +
                " ( _id INTEGER PRIMARY KEY AUTOINCREMENT, TITLE TEXT, DETAIL TEXT, " +
                "ISDONE INTEGER, DAY INTEGER, MONTH INTEGER, YEAR INTEGER, ISTIMED INTEGER, TIME TEXT)");

        db.execSQL("CREATE TABLE IF NOT EXISTS " + PROJECT_TABLE_NAME +
                " ( _id INTEGER PRIMARY KEY AUTOINCREMENT, TITLE TEXT, DETAIL TEXT," +
                " TEAM INTEGER, DAY INTEGER, MONTH INTEGER, YEAR INTEGER, ROLE TEXT," +
                " ASSIGNEDTASK TEXT, PERCENT INTEGER)");

        db.execSQL("CREATE TABLE IF NOT EXISTS " + CONTACT_TABLE_NAME +
                " ( _id INTEGER PRIMARY KEY AUTOINCREMENT, NAME TEXT, SURNAME TEXT, " +
                "NICK TEXT, EMAIL TEXT, PHONE TEXT, FACEBOOK TEXT, TWITTER TEXT, INSTAGRAM, TEXT, SKILL TEXT)");
    }
    ...
    /////////////////contact
    public boolean insertContactdata(String name, String surname, String nick,
                                     String email, String phone, String facebook, String twitter,
                                     String instagram, String skill) {
    ...
    ///////////////////////
    public boolean editproject(String id, String title, String detail, String team,
                               String day, String month, String year, String role, String task) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
...
/////
    ...
    /////
    public boolean insertPRJdata(String title, String detail, String team, String role,
                                 String task, String day, String month, String year) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    /////
    public boolean insertTODOdata(String title, String description, String stime,
                                  String etime, String istimed, String day, String month, String year) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    ///////////formerly to-do fragment but now used as today fragment, same with to-do
    /// opportune which is now for tomorrow fragment
    public Cursor getTODOdata(int day, int month, int year) {
        Calendar daydate = Calendar.getInstance();


        //dates are stored normally(january starting form 1) in database and can be
        // compared with the SQL format which also starts from 1.
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//The YYYY-MM-DD DATE is a s
        // tandard SQL data type, take note MM is month not mm

        String todayd = sdf.format(daydate.getTime());

        SQLiteDatabase db = this.getWritableDatabase();

        //  Cursor res=db.rawQuery("SELECT * from "+TABLE_NAME+" WHERE DAY="+day+"
        // AND MONTH="+month+" AND YEAR="+year+" AND ISDONE='0'",null);
    ...
    public Cursor getcurrentdayTask(int day, int month, int year)//for home page
    {
        ...
        //dates are stored normally(january starting form 1) in database and can be
        // compared with the SQL format which also starts from 1.
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//The YYYY-MM-DD DATE is
        // a standard SQL data type, take note MM is month not mm
    ...
    public Cursor getnextdayTask(int day, int month, int year)//for home page
    {
        ...
        //dates are stored normally(january starting form 1) in database and can be
        // compared with the SQL format which also starts from 1.
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//The YYYY-MM-DD
        // DATE is a standard SQL data type, take note MM is month not mm
    ...
    public Cursor getnextsevendays() {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//The YYYY-MM-DD DATE is a
        // standard SQL data type, take note MM is month not mm
        ...
        Cursor res = db.rawQuery("SELECT * from " + TABLE_NAME + " WHERE TDATE BETWEEN '" +
                todayd + "' AND '" + next7d + "' ORDER BY TDATE ASC", null);
        //sample date 2017-02-11, order aranges tasks so user can understand better
    ...
    public Cursor getnext4weeks() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + TABLE_NAME + " WHERE TDATE BETWEEN '" +
                todayd + "' AND '" + next7d + "' ORDER BY TDATE ASC", null);//sample date 2017-02-11,
        // order aranges tasks so user can understand better
    ...
    //
    public boolean insertCLASSdata(String title, String code, String credit, String venue,
                                   String lecturers, String starttime, String stoptime, String
                                           originaltime, String day) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public Cursor getmondayclasss() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Monday' ORDER BY ORIGINALTIME ASC", null);
    ...
    public Cursor gettuesdayclass() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Tuesday' ORDER BY ORIGINALTIME ASC", null);
    ...
    public Cursor getwednesdayclass() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Wednesday' ORDER BY ORIGINALTIME ASC", null);
    ...
    public Cursor getthursdayclass() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Thursday' ORDER BY ORIGINALTIME ASC", null);
    ...
    public Cursor getfridayclass() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Friday' ORDER BY ORIGINALTIME ASC", null);
    ...
    public Cursor getssaturdayclass() {
        ...
        Cursor res = db.rawQuery("SELECT * from " + CLASSES_TABLE_NAME +
                " WHERE DAY_C='Saturday' ORDER BY ORIGINALTIME ASC", null);
